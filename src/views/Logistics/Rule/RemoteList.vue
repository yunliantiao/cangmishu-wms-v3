...
const resultType = (type) => {
  if (type === 'remote') {
    return '偏远地区';
  } else if (type === 'very_remote') {
    return '超偏远地区';
  } else if (type === 'super_remote') {
    return '超级偏远地区';
  } else if (type === 'ultra_remote') {
    return '超超偏远地区';
  } else {
    return '-';
  }
...
const onDelete = (row) => {
  $q.dialog({
    ...
    .onOk(() => {
      // 调用接口删除数据
      logisticsApi.delRemote(row.id).then((res) => {
        // $q.notify({
        //   type: 'positive',
        //   message: '删除成功',
        // });
...
const getRemoteList = () => {
  logisticsApi.getRemoteList(pageParams).then((res) => {
    remoteList.value = res.data.items;
