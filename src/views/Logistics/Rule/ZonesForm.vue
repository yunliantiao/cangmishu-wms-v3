      ...
      <q-card-section>
        ...
        <q-table flat :rows="form.rules" :columns="columns" row-key="id" :key="tableKey" hide-bottom>
          <template v-slot:body-cell-code="props">
            <q-td>
              <q-input v-model="props.row.code" dense outlined placeholder="请输入" class="input" :disable="isEdit" />
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-city="props">
            <q-td>
              <q-input v-model="props.row.city" dense outlined placeholder="请输入" class="input" :disable="isEdit" />
            </q-td>
          </template>
          <template v-slot:body-cell-postcode_start="props">
            <q-td>
              <q-input
                v-model="props.row.postcode_start"
                dense
                outlined
                placeholder="请输入"
                class="input"
          ...
          <template v-slot:body-cell-postcode_end="props">
            <q-td>
              <q-input
                v-model="props.row.postcode_end"
                dense
                outlined
                placeholder="请输入"
                class="input"
...
const columns = [
  { name: 'code', label: '分区代码', align: 'left', field: 'code' },
...
const getAddForm = () => {
  logisticsApi.addZones(form.value).then((res) => {
    router.back();
  });
};
const getEditForm = () => {
  logisticsApi.updateZones(remoteId, form.value).then((res) => {
    router.back();
  });
};
const getDetailForm = () => {
  logisticsApi.getZonesInfo(remoteId).then((res) => {
    form.value = res.data;
  });
};
